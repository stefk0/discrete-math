...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    % \State $t := t+1$
    % \State $in[u] := t$
    \State $color[u] := GRAY$\Comment{Върхът $u$ е посетен, но не е обработен}
    \ForAll{$v \in Adj[u]$}
    \If {$WHITE = color[v]$}
    \State $pred[v] := u$
    ...
    % \State $t := t+1$
    % \State $out[u] := t$
    \State $color[u] := BLACK$\Comment{Приключили сме с $u$}
    \EndProcedure
    \Statex
    \Procedure{DFS}{}
...
\begin{algorithm}[h!]
  ...
  \begin{algorithmic}[1]
    \Procedure{KRUSKAL}{}
    \State $X = \emptyset$\Comment{$X$ ще бъде списък с дървета}
    \For{$v \in V$}
