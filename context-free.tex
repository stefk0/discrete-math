...
\begin{itemize}
\item
  $\Ls_F(P)$ е езика, който се разпознава от $P$ с финално състояние,
  \[\Ls_F(P) = \{w\mid (q_0,w,\varepsilon) \vdash^\star_P (q,\varepsilon,\alpha)\ \&\ q \in F\}.\]    
\item
  $\Ls_S(P)$ е езика, който се разпознава от $P$ с празен стек,
  \[\Ls_S(P) = \{w\mid (q_0,w,\varepsilon) \vdash^\star_P (q,\varepsilon,\varepsilon)\}.\]    
\end{itemize}
...
\begin{problem}
  ...
  Постройте стеков автомат $\A$, който разпознава с празен стек
  и такъв, че $L(\A) = L(G)$, където правилата $R$ на граматиката са зададени като:
  \begin{enumerate} 
